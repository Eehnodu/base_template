...
const Pagination = ({
  ...
  const normal = "border-line text-text-main bg-bg-card hover:bg-bg-hover";
  ...
  const nav = (
    <nav className="flex items-center gap-2 select-none" aria-label="Pagination">
      <button
        type="button"
        className={`${base} ${sizeClass} ${normal} ${start === 1 ? disabled : ""}`}
      ...
      <button
        type="button"
        className={`${base} ${sizeClass} ${normal} ${end === totalPages ? disabled : ""}`}
